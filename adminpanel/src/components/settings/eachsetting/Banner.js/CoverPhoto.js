...
const CoverPhoto = () => {
	const { coverimage, setcoverimage, fetchCover } = useContext(MyContext);
	...
	const deleteProductImages = (productsImagefilename) => {
		...
		const desertRef = ref(storage, `/cover/${productsImagefilename}`);
	...
	const BannerSubmit = (e) => {
		...
		if (coverimage.length !== 0) {
			deleteProductImages(coverimage[0].kioskCover.filename);
		}
		if (
			banner.comf1 === null ||
		...
		const storageRef1 = ref(storage, "cover/" + banner.comf1.name);
	...
	const SubmitBannerDetails = async () => {
		try {
			...
			if (coverimage.length === 0) {
				...
				await addDoc(collection(db, "kioskcover"), {
					kioskCover: {
			...
			const formRef = doc(db, "kioskcover", coverimage[0].cid);
			...
			const uu = coverimage.map((pp) => {
				if (pp.id === coverimage[0].cid) {
					pp.kioskCover.filename = bannername.cf1;
					pp.kioskCover.url = imageurl.url;
					pp.kioskCover.color = filecolor.cl1;
				}
				return pp;
			});
			setcoverimage(uu);
				...
				{coverimage.length !== 0
					? coverimage.map((b) => (
							<div
								...
								<div
									style={{
										backgroundColor: `${b.kioskCover.color}`,
									}}
